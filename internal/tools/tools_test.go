...
func TestDefinitionsMatchCurrentAgentPyContract(t *testing.T) {
	...
	tests := []struct {
		...
		{index: 4, name: "grep", description: "Search file contents by regular expression (Go regexp) when locating unknown symbols, files, or usages. Returns path:line:text. Skips .git and binary files.", required: []string{"pattern"}},
