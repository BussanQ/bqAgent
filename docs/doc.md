...
“新会话”菜单支持普通会话和群聊。群聊使用独立的 `conversation_type=group` session，由 bqagent 作为调度员，成员名单在首次发送时固定为 bqagent 加当前检测可用的外部 Agent。`@成员` 是硬路由且按出现顺序执行；无 @ 时调度员可以通过 `consult_group_agent` 自主邀请或追问成员。所有成员使用同一 workspace，前序成员结论会加入后续成员的受预算约束共享上下文，并通过 `participant_start`、`participant_message`、`participant_error` SSE 事件单独展示，最后的普通 `message`/`done` 仍由 bqagent 汇总。每个外部成员在同一群聊下保留独立外部 session。群聊仅允许 Run，TUI 暂不提供群聊入口。
