...
[internal/agent/loop.go](internal/agent/loop.go) `runConversation` is the heart of the system. The CLI iteration cap is a **runaway safety valve, not a task limit**: `agent.DefaultMaxIterations = 1000` is the in-loop fallback and the default for `runtime.MaxIterations` (env `AGENT_MAX_ITERATIONS`). Message channels default to `30` iterations and can be overridden with `CHANNEL_AGENT_MAX_ITERATIONS`; QQ and WeChat/iLink additionally use `CHANNEL_STAGE_*` checkpoint budgets (default 20 iterations / 90s). WebUI instead uses the global runaway limit and has no fixed stage budget by default; positive `WEBUI_STAGE_*` values opt back into checkpoint boundaries. Each iteration:
...
**Context management** (`buildRequestMessages`): it converts completed tool scaffolding into assistant evidence that retains tool names, arguments, and results, prunes to a token budget, and applies a hard request-size guard when a recent turn or tool result alone is oversized. The hard guard prioritizes the system prompt, conversation summary, and latest user request, then fits or truncates recent evidence. Token budgeting prefers the previous response's provider-reported prompt usage, estimates only the newly appended messages plus request shape locally, and asks providers with a count endpoint for an exact full-request count near the configured boundary; unsupported or failed count calls fall back to the local `chars/4` estimate. When over `SummaryTriggerTokens` and summarization is enabled (**on by default**) it summarizes older turns into a synthetic message and writes `context_checkpoint.json`. The full on-disk `messages.jsonl` remains an audit trail and is not sent verbatim to the model. All tunable via `CONTEXT_*` env vars (see `runtime.ConfigFromEnv`).
...
`Service.HandleTurn` orchestrates one turn for any caller and is guarded by a **per-session keyed lock** so concurrent requests to the same session serialize. A caller may assign `TurnRequest.TurnID`; the shared service then registers the whole turn for cancellation through `Service.StopTurn` / `POST /api/v1/chat/stop`, independent of any channel. Channels implement the `Channel` interface (`Name/Enabled/RegisterRoutes/Start`) and are registered in [cmd/agent/server.go](cmd/agent/server.go); each is enabled by its own env vars. The **web UI channel** ([internal/server/webui.go](internal/server/webui.go)) is **on by default** (`WEBUI_ENABLED=false` to disable): it serves a self-contained `go:embed`-ed page at `/`, streams replies token-by-token, emits detailed `progress` SSE events for iterations and tools, and currently supplies the stop UI. WebUI defaults to one uninterrupted request with no stage or whole-turn timeout; browser disconnect, explicit stop, loop protection, and the global runaway limit still cancel or bound it. Positive `WEBUI_STAGE_*` values opt back into persisted checkpoints. QQ suppresses internal iteration/tool progress but retains short long-wait notices, loop protection, and final persisted checkpoints. WeChat/iLink also suppresses intermediate progress because its context token must be reserved for the single final reply. Their stage budgets produce an assistant checkpoint (`已发现 / 未完成 / 建议下一步`), after which “继续” resumes the same session, and `CHANNEL_TURN_TIMEOUT` (default 10m) bounds the whole message-channel turn. Streaming LLM HTTP requests have no `http.Client` total timeout and remain bounded by their request context (disconnect, explicit stop, configured stage deadline, or shutdown); non-streaming LLM requests retain the default two-minute client timeout.
